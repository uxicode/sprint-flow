...
async function fetchCalendarEvents(accessToken, calendarId, timeMin, timeMax) {
  ...
  if (!response.ok) {
    ...
    const error = new Error(googleErrorMsg);
  ...
  try {
    ...
    if (action === 'events') {
      ...
      try {
        ...
        // 401인 경우 refresh 재시도
        if ((fetchErr.status === 401 || fetchErr.message.includes('401')) && refreshToken && clientId && clientSecret) {
          try {
            const refreshed = await refreshAccessToken(refreshToken, clientId, clientSecret);
